...
export default function EditContact({ open, action, data, setData, list, updateList }) {
    ...
    <Box width="100%" mb={2}>
        <Dialog
            ...
            <DialogContent>
                <Box mb={2}>
                    <TextField fullWidth name="name" disabled={isLoading} inputProps={{ maxLength: 30 }} placeholder="Enter name" value={formData === null ? '' : formData.name} onChange={(e) => onChange(e)}/>
                </Box>
                <Box mb={2}>
                    <TextField fullWidth name="email" disabled={isLoading} inputProps={{ maxLength: 40 }} placeholder="Enter email" value={formData === null ? '' : formData.email} onChange={(e) => onChange(e)}/>
                </Box>
